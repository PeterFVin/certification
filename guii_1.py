...
def on_ok():
    ...
    for sheet_name, sheet_frame in sheets.items():
        ...
        variables[f'{{CONTRACT_NUMBER_{sheet_name}}}'] = entries['contract_number'].get()
        variables[f'{{CONTRACT_YEAR_{sheet_name}}}'] = entries['contract_year'].get()
        variables[f'{{COMPANY_NAME_FULL_{sheet_name}}}'] = entries['company_name_full'].get()
...
def create_sheet():
    ...
    tk.Label(frame, text="COMPANY_NAME_FULL:").grid(row=2, column=0, padx=10, pady=5)
    entries['company_name_full'] = tk.Entry(frame)
    entries['company_name_full'].grid(row=2, column=1, padx=10, pady=5)
