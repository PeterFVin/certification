from pathlib import Path

from docx import Document
from docx.enum.text import WD_ALIGN_PARAGRAPH
from docx2pdf import convert
import openpyxl

from func_genitive import genitive, genitive_name, number_to_words


MONTHS = {
    '01': 'январь',
    '02': 'февраль',
    '03': 'март',
    '04': 'апрель',
    '05': 'май',
    '06': 'июнь',
    '07': 'июль',
    '08': 'август',
    '09': 'сентябрь',
    '10': 'октябрь',
    '11': 'ноябрь',
    '12': 'декабрь',
}

theFile = openpyxl.load_workbook('Инспекция Исходник Монолит.xlsx')


def load_variables_from_sheet(excel_file_path, sheet_number):
    currentSheet = theFile.worksheets[sheet_number]

    variables = {
        '{{IK_NUMBER}}': str(currentSheet['B2'].value),
        '{{IK_CONTRACT_NUMBER}}': str(currentSheet['B3'].value),
        '{{IK_CONTRACT_YEAR}}': str(currentSheet['B4'].value),  # Короткий год - 24
        '{{WORK_NUMBER}}': str(currentSheet['B5'].value),
        # '{{BILL_NUMBER}}': str(currentSheet['B5'].value),  # Короткий год - 24
        '{{IK_CONTR_DATE}}': str(currentSheet['B6'].value),
        '{{IK_CONTR_MONTH}}': str(currentSheet['B7'].value),
        '{{IK_CONTR_MONTH_WORD_GEN}}': genitive(MONTHS[str(currentSheet['B7'].value)]),
        '{{IK_CONTR_YEAR}}': str(currentSheet['B8'].value),  # Длинный год - 2024
        # '{{BUSINESS_FORM_FULL}}': str(currentSheet['B8'].value),
        # '{{COMPANY_NAME_FULL}}': str(currentSheet['B9'].value),
        '{{BUSINESS_FORM}}': str(currentSheet['B9'].value),
        '{{COMPANY_NAME}}': str(currentSheet['B10'].value),
        '{{DIR_LASTNAME}}': str(currentSheet['B11'].value),  # Фамилия
        # '{{DIR_FIRSTNAME}}': str(currentSheet['B13'].value),  # Имя
        # '{{DIR_SECNAME}}': str(currentSheet['B14'].value),  # Отчество
        '{{DIR_FIRSTNAME_SHORT}}': str(currentSheet['B12'].value)[:1] + '.',
        '{{DIR_SECNAME_SHORT}}': str(currentSheet['B13'].value)[:1] + '.',
        # '{{DIR_LASTNAME_GEN}}': genitive_name('LASTNAME', str(currentSheet['B15'].value), str(currentSheet['B12'].value)),  # Фамилия
        # '{{DIR_FIRSTNAME_GEN}}': genitive_name('FIRSTNAME', str(currentSheet['B15'].value), str(currentSheet['B13'].value)),  # Имя
        # '{{DIR_SECNAME_GEN}}': genitive_name('MIDDLENAME', str(currentSheet['B15'].value), str(currentSheet['B14'].value)),  # Отчество
        # !!!!!!!!НОВЫЙ!!!!!!!!!!!
        '{{CERT_NUMBER}}': str(currentSheet['B14'].value),
        '{{CERT_DATE}}': str(currentSheet['B15'].value),
        '{{CERT_MONTH}}': str(currentSheet['B16'].value),
        '{{CERT_MONTH_WORD_GEN}}': genitive(MONTHS[str(currentSheet['B16'].value)]),
        '{{CERT_YEAR}}': str(currentSheet['B17'].value),
        '{{CERT_NAME}}': str(currentSheet['B18'].value),
        # '{{CERT_GROUP}}': str(currentSheet['B17'].value),
        '{{OKPD}}': str(currentSheet['B19'].value),
        # '{{STANDART_MAIN}}': str(currentSheet['B19'].value),
        # '{{STANDART_SHORT}}': str(currentSheet['B20'].value),
        '{{STANDART_FULL}}': str(currentSheet['B20'].value),
        # '{{CONTRACT_SUM}}': str(currentSheet['B22'].value),
        # '{{CONTRACT_OS_FULL_SUM}}': str(currentSheet['B23'].value),
        # '{{CONTRACT_OS_SUM}}': str(currentSheet['B24'].value),
        # '{{CONTRACT_IL_FULL_SUM}}': str(currentSheet['B25'].value),
        # '{{CONTRACT_IL_SUM}}': str(currentSheet['B26'].value),
        '{{CONTRACT_IK_SUM}}': str(currentSheet['B21'].value),
        '{{CONTRACT_IK_SUM_WORDS}}': number_to_words(str(currentSheet['B22'].value)),
        # !!!!!!!!НОВЫЙ!!!!!!!!!!!
        '{{CONTRACT_IK_OS_FULL_SUM}}': str(currentSheet['B22'].value),
        '{{CONTRACT_IK_OS_SUM}}': str(currentSheet['B23'].value),
        '{{INN}}': str(currentSheet['B24'].value),
        '{{KPP}}': str(currentSheet['B25'].value),
        '{{JUR_ADDRESS}}': str(currentSheet['B26'].value),
        # '{{PHYS_ADDRESS}}': str(currentSheet['B31'].value),
        '{{RAS_ACCOUNT}}': str(currentSheet['B27'].value),
        '{{BANK_NAME}}': str(currentSheet['B28'].value),
        '{{KORR_ACCOUNT}}': str(currentSheet['B29'].value),
        '{{BIK}}': str(currentSheet['B30'].value),
        '{{OGRN}}': str(currentSheet['B31'].value),
        '{{TEL}}': str(currentSheet['B32'].value),
        # !!!!!!!!НОВЫЕ!!!!!!!!!!!
        '{{IK_START_DATE}}': str(currentSheet['B33'].value),
        '{{IK_START_MONTH}}': str(currentSheet['B34'].value),
        '{{IK_START_MONTH_WORD_GEN}}': genitive(MONTHS[str(currentSheet['B34'].value)]),
        '{{IK_START_MONTH+1}}': '01' if str(currentSheet['B34'].value) == '12' else str(int(currentSheet['B34'].value) + 1),  # Для указания месяца проведения работ.
        '{{IK_START_YEAR}}': str(currentSheet['B35'].value),  # Длинный год - 2024
        '{{IK_START_YEAR+1}}': str(currentSheet['B35'].value) if str(currentSheet['B33'].value) != '12' else '20' + str(int(currentSheet['B35'].value) + 1),  # Для указания года окончания проведения работ.
        '{{IK_END_DATE}}': str(currentSheet['B36'].value),
        '{{IK_END_MONTH}}': str(currentSheet['B37'].value),
        '{{IK_END_MONTH_WORD_GEN}}': genitive(MONTHS[str(currentSheet['B37'].value)]),
        '{{IK_END_YEAR}}': str(currentSheet['B38'].value),  # Длинный год - 2024


        # '{{E-MAIL}}': str(currentSheet['B38'].value),
        # '{{EXPERT_LASTNAME}}': str(currentSheet['B39'].value),  # Фамилия
        # '{{EXPERT_FIRSTNAME_SHORT}}': str(currentSheet['B40'].value),
        # '{{EXPERT_SECNAME_SHORT}}': str(currentSheet['B41'].value),
        # '{{EXPERT_REG_NUMBER}}': str(currentSheet['B42'].value),
        # '{{IL_NAME}}': str(currentSheet['B43'].value),
        # '{{IL_REG_NUMBER}}': str(currentSheet['B44'].value),
        # '{{IL_EXPIRE_DATE}}': str(currentSheet['B45'].value),
        # '{{ISSUE_DECISION_DATE}}': str(currentSheet['B46'].value),
        # '{{ISSUE_DECISION_MONTH}}': str(currentSheet['B47'].value),
        # '{{ISSUE_DECISION_MONTH_GEN}}': genitive(MONTHS[str(currentSheet['B47'].value)]),
        # '{{ISSUE_DECISION_YEAR}}': str(currentSheet['B48'].value),
        # '{{CERTIFICARTE_START_DATE}}': str(currentSheet['B49'].value),
        # '{{CERTIFICARTE_START_MONTH}}': str(currentSheet['B50'].value),
        # '{{CERTIFICARTE_START_YEAR}}': str(currentSheet['B51'].value),
        # '{{CERTIFICARTE_DURATION}}': str(currentSheet['B52'].value),
        # '{{SAMPLE_ACT_DATE}}': str(currentSheet['B53'].value),
        # '{{SAMPLE_ACT_MONTH}}': str(currentSheet['B54'].value),
        # '{{SAMPLE_ACT_MONTH_GEN}}': genitive(MONTHS[str(currentSheet['B54'].value)]),
        # '{{SAMPLE_ACT_YEAR}}': str(currentSheet['B55'].value),
        # '{{PRODUCTION_CREATED_QUARTER}}': str(currentSheet['B56'].value),
        # '{{PRODUCTION_CREATED_YEAR}}': str(currentSheet['B57'].value),
        # '{{PRODUCTION_SAMPLES}}': str(currentSheet['B58'].value),
        # '{{PROTOCOL_DATE}}': str(currentSheet['B59'].value),
        # '{{PROTOCOL_MONTH}}': str(currentSheet['B60'].value),
        # '{{PROTOCOL_MONTH_WORD_GEN}}': genitive(MONTHS[str(currentSheet['B60'].value)]),
        # '{{PROTOCOL_YEAR}}': str(currentSheet['B61'].value),
        # '{{PROTOCOL_NUMBER}}': str(currentSheet['B62'].value),
        # '{{TEST_GOSTS}}': str(currentSheet['B63'].value),
        # '{{TEST_START_DATE}}': str(currentSheet['B64'].value),
        # '{{TEST_START_MONTH}}': str(currentSheet['B65'].value),
        # '{{TEST_START_YEAR}}': str(currentSheet['B66'].value),
        # '{{SAMPLES_MARK}}': str(currentSheet['B67'].value),
        # '{{TESTER_NAME}}': str(currentSheet['B68'].value),
        # '{{PROD_ANALYSE_DATE}}': str(currentSheet['B69'].value),
        # '{{PROD_ANALYSE_MONTH}}': str(currentSheet['B70'].value),
        # '{{PROD_ANALYSE_YEAR}}': str(currentSheet['B71'].value),
        'worksheets_count': len(theFile.worksheets),
    }

    return variables


def main(template_file_path, output_file_path, variables):
    template_document = Document(template_file_path)

    for variable_key, variable_value in variables.items():
        for paragraph in template_document.paragraphs:
            replace_text_in_paragraph(paragraph, variable_key, variable_value)

        for table in template_document.tables:
            for row in table.rows:
                for cell in row.cells:
                    for paragraph in cell.paragraphs:
                        replace_text_in_paragraph(paragraph, variable_key, variable_value)

    template_document.save(output_file_path)
    print('OK!!!!!!!!!!!!!!!!!!!', output_file_path)


def replace_text_in_paragraph(paragraph, key, value):
    if key in paragraph.text:
        text_runs = [(run.text, run.font.name, run.font.size, run.bold, run.italic, run.underline) for run in paragraph.runs]
        full_text = ''.join([text for text, _, _, _, _, _ in text_runs])
        new_text = full_text.replace(key, value)

        # Clear existing runs
        for run in paragraph.runs:
            run.text = ''

        # Create a new run to hold the entire new text
        new_run = paragraph.add_run(new_text)

        # Apply formatting from the first run
        if text_runs:
            _, font_name, font_size, bold, italic, underline = text_runs[0]
            new_run.font.name = font_name or 'Times New Roman'
            new_run.font.size = font_size
            new_run.bold = bold
            new_run.italic = italic
            new_run.underline = underline


def reference_data_xlsx(input_reference, output_reference, variables):
    theFile = openpyxl.load_workbook(input_reference)
    currentSheet = theFile['На регистрацию ИК']
    currentSheet['A4'].value = f"на регистрацию результатов ИК-{variables['{{IK_NUMBER}}']}"
    currentSheet['A6'].value = f"по сертификату соответствия № {variables['{{CERT_NUMBER}}']}"
    currentSheet['D9'].value = f"ИК{variables['{{IK_NUMBER}}']}-{variables['{{IK_CONTRACT_YEAR}}']}-{variables['{{WORK_NUMBER}}']} от {variables['{{IK_START_DATE}}']}.{variables['{{IK_START_MONTH}}']}.{variables['{{IK_START_YEAR}}']}г."
    currentSheet['D11'].value = f"{variables['{{BUSINESS_FORM}}']} «{variables['{{COMPANY_NAME}}']}»"
    currentSheet['D13'].value = variables['{{CERT_NAME}}']
    currentSheet['D14'].value = f"ИК{variables['{{IK_NUMBER}}']}-{variables['{{IK_CONTRACT_YEAR}}']}-{variables['{{WORK_NUMBER}}']}"
    currentSheet['D15'].value = f"{variables['{{IK_END_DATE}}']}.{variables['{{IK_END_MONTH}}']}.{variables['{{IK_END_YEAR}}']}г."
    currentSheet['D24'].value = f"{variables['{{IK_CONTRACT_NUMBER}}']}-1-{variables['{{IK_CONTRACT_YEAR}}']}"
    currentSheet['D25'].value = f"{variables['{{IK_CONTR_DATE}}']}.{variables['{{IK_CONTR_MONTH}}']}.{variables['{{IK_CONTR_YEAR}}']}г."
    currentSheet['D26'].value = variables['{{CONTRACT_IK_SUM}}']
    currentSheet['D27'].value = variables['{{CONTRACT_IK_OS_FULL_SUM}}']
    currentSheet['D28'].value = variables['{{CONTRACT_IK_OS_SUM}}']
    
    theFile.save(output_reference)



if __name__ == '__main__':
    excel_file_path = 'Инспекция Заготовка.xlsx'
    currentSheet = theFile.worksheets[0]
    variables = load_variables_from_sheet(excel_file_path, sheet_number=0)
    company = variables['{{COMPANY_NAME}}']
    BASE_DIR = Path(__file__).parent
    main_dir = BASE_DIR / f'{company}'
    main_dir.mkdir(exist_ok=True)
    main_dir = BASE_DIR / f'{company}' / 'Договора'
    main_dir.mkdir(exist_ok=True)
    main_dir = BASE_DIR / f'{company}' / 'Комплект документов'
    main_dir.mkdir(exist_ok=True)
    works = len(theFile.worksheets)
    if works > 1:
        main_dir = BASE_DIR / f'{company}' / 'Комплект документов' / '01 Распоряжения'
        main_dir.mkdir(exist_ok=True)
        main_dir = BASE_DIR / f'{company}' / 'Комплект документов' / '02 Акты оценки производства'
        main_dir.mkdir(exist_ok=True)
        main_dir = BASE_DIR / f'{company}' / 'Комплект документов' / '03 Акты инспекционного контроля'
        main_dir.mkdir(exist_ok=True)
        main_dir = BASE_DIR / f'{company}' / 'Комплект документов' / '04 Справки'
        main_dir.mkdir(exist_ok=True)
    main('03 АКТ приемки Исходник.docx', f"{company}/Договора/03 АКТ приемки {company}.docx", variables)
    works = len(theFile.worksheets)
    for work in range(works):
        sheet_number = work
        variables = load_variables_from_sheet(excel_file_path, sheet_number)
        # add - добавка -1, -2 и т.д. к номеру работы, если больше одной работы
        add = ('' if works <= 1 else '-' + str(work+1))
        main('01 Распоряжение Исходник.docx', f"{company}/Комплект документов{'/01 Распоряжения' if works > 1 else ''}/01{add} Распоряжение {company}.docx", variables)
        main('02 Акт оценки производства Исходник.docx', f"{company}/Комплект документов{'/02 Акты оценки производства' if works > 1 else ''}/02{add} Акт оценки производства {company}.docx", variables)
        main('03 Акт инспекционного контроля Исходник.docx', f"{company}/Комплект документов{'/03 Акты инспекционного контроля' if works > 1 else ''}/03{add} Акт инспекционного контроля {company}.docx", variables)
        reference_data_xlsx('04 Справка Исходник.xlsx', f"{company}/Комплект документов{'/04 Справки' if works > 1 else ''}/04{add} Справка {company}.xlsx", variables)
